...
#include "ObstacleManager.h" // Use the new manager
...
static std::unique_ptr<ObstacleManager> obstacleManager; // Changed to ObstacleManager
...
static void display(){
    ...
    if(obstacleManager) obstacleManager->draw(); // Draw the obstacles
    glutSwapBuffers();
}
...
static void key(unsigned char c, int x, int y){
    mx = x; my = y; // Update mouse position for key presses
    switch(c){
        case 'c': case 'C':
            grid.reset();
            if (obstacleManager) obstacleManager->clear();
            break;
        case 'v': case 'V': showVel=!showVel; break;
        case 'q': case 'Q': std::exit(0);    break;
        case '1': { // Add a new fixed rectangle at the mouse position
            if (obstacleManager) {
                int i = int(( mx / float(winX))*N + 1);
                int j = int(((winY-my)/float(winY))*N + 1);
                obstacleManager->addRect(i - 2, j - 2, 5, 5);
            }
            break;
        }
    }
...
int main(int argc,char** argv){
    if(argc!=1 && argc!=7){
        std::fprintf(stderr, "usage: %s [N dt diff visc force source]\n",argv[0]);
        return 1;
    }
    if(argc==7){
        N = std::atoi(argv[1]); dt = std::atof(argv[2]); diff = std::atof(argv[3]);
        visc = std::atof(argv[4]); cmd_force = std::atof(argv[5]); cmd_source = std::atof(argv[6]);
    }
    if (N < 8 || N > 1024) {
    ...
    // Create and configure the obstacle manager
    obstacleManager.reset(new ObstacleManager(N));
    obstacleManager->addRect(N / 4, N / 4, 5, N / 2); // Add an initial wall
    ...
    std::puts("\nControls:\n"
              ...
              "  1           : add a solid block at mouse location\n" // New control
